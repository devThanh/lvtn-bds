...
//Generate verify code (0000-9999)
export const random = Math.floor(Math.random() * 9000 + 1000)

const mailOptions = {
    from: 'vantranthanhh@gmail.com',
    to: '',
    subject: '',
    text: '',
    html: ``,
...
export async function senMailer(email: string) {
    mailOptions.to = email
    mailOptions.subject = `Hi ${email}`
    mailOptions.text = `Your account has been create successfully!`
    mailOptions.html = `<p>Verify code: ${random}</p><br> Please click on the link to verify your account.<br><a href="http://localhost:8080/user/verify?email=${email}&code=${random}">Click here to verify</a>"`
...
export async function senMailerForgetPass(email: string, pass: string) {
    mailOptions.to = email
    mailOptions.subject = `Hi ${email}`
    mailOptions.text = `Your account has been create successfully!`
    mailOptions.html = `<p>Your password is: ${pass}.</br>Please change password</p>`
    await transporter.sendMail(mailOptions, async function (error, info) {
        if (error) {
            console.log(error)
        } else {
...
export async function senMailerRePost(email: string, real_easte_id: string, expiration: number, approval_date: string, name: string) {
    mailOptions.to = email
    mailOptions.subject = `Hi ${name}`
    mailOptions.text = `THÔNG BÁO`
    mailOptions.html = `<h3>Cảm ơn bạn đã tin tưởng website sàn giao dịch bất động sản Thanh Build</h3> </br><p>Tin của bạn ( mã tin: ${real_easte_id} ) đã được đăng lại vào ngày ${approval_date} và sẽ hết hạn sau ${expiration} ngày. </br> Cảm ơn</p>`
    await transporter.sendMail(mailOptions, async function (error, info) {
        if (error) {
            console.log(error)
        } else {
